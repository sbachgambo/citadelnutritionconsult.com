document.addEventListener('DOMContentLoaded', function () {
	...
	// Safety net: heavy sub-content can occasionally starve IntersectionObserver
	// callbacks on the main thread, so anything that never gets flagged as
	// intersecting must still reveal itself after a short delay rather than
	// stay invisible forever.
	var sections = document.querySelectorAll('main.wp-block-group > section, main.wp-block-group > .wp-block-group');
	if (sections.length) {
		sections.forEach(function (section, i) {
			section.classList.add('cnc-reveal');
	...
	// Mouse-tracking 3D tilt on large photography.
