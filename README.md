...
- **Blog** — 3 real posts (title, excerpt, real featured image) seeded as standard Posts, plus a real `/blog/` listing page now exists (`page-blog.html` template + `blog-full.php` pattern, a 3-column card grid matching the site's visual style) — verified live: real titles, responsive images, and excerpts all render correctly with zero errors. `single.html` (individual post pages) also verified live and working. Two things to do before launch: delete WordPress's default "Hello world!" post (it still shows in the Blog listing), and migrate in the full article body text — only the excerpt is seeded, with a content-editor comment linking back to the source URL as a reminder.
